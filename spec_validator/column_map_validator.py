...
def check_column_map(column_map_path, column_list_path, spec_path, output_path='../output/', delimiter='!!'):
	...
	for year in column_map:
		...
		missing_columns = list(set(column_year_list[year])-set(cur_column_list[year]))

		if len(missing_columns) > 0:
			stat_dir[year]['missing_columns'] = missing_columns

		# column should have been ignored
		extra_columns = list(set(cur_column_list[year])-set(column_year_list[year]))
	...
	# missing columns in output
	missing_columns = list(set(column_year_list['all'])-set(cur_column_list['all']))
	...
	# column should have been ignored
	extra_columns = list(set(cur_column_list['all'])-set(column_year_list['all']))
	...
	json.dump(stat_dir, open(os.path.join(table_dir+'validation_column_map.json'), 'w'), indent=2)
