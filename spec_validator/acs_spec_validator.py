...
def find_columns_with_no_properties(column_name_list, spec_dict, delimiter='!!'):
	ret_list = []
...
# assumes columnNameList does not contain columns to be ignored
def find_missing_enum_specialisation(column_name_list, spec_dict, delimiter='!!'):
	ret_dict = {}
	for column_name in column_name_list:
		temp_dict = {}
			...
			if len(temp_dict[prop]) > 1:
				...
				for i, prop_token in enumerate(reversed(temp_dict[prop])):
					...
					if 'enumSpecializations' in spec_dict:
						if token_in_list_ignore_case(prop_token, spec_dict['enumSpecializations']):
							temp_flag = False
							if spec_dict['enumSpecializations'][prop_token] not in temp_dict[prop][:j]:
								if prop_token not in ret_dict:
									...
									ret_dict[prop_token]['possibleParents'].extend(temp_dict[prop][:j])
	...
	for prop_token in ret_dict:
		ret_dict[prop_token]['possibleParents'] = list(set(ret_dict[prop_token]['possibleParents']))
...
# assumes columnNameList does not contain columns to be ignored
def find_missing_denominator_total_column(column_name_list, spec_dict, delimiter = '!!'):
	ret_list = []
...
def test_column_name_list(column_name_list, spec_dict, test_list=['all'], raise_warnings_only = False, delimiter = '!!'):
	ret_dict = {}

	# remove ignore columns
	column_name_list = remove_columns_to_be_ignored(column_name_list, spec_dict, delimiter)
	...
	if 'all' in test_list or 'denominators' in test_list:
		temp_list = find_missing_denominator_total_column(column_name_list, spec_dict, delimiter)
		...
		temp_list = find_missing_denominators(column_name_list, spec_dict, delimiter)
		...
		temp_list = find_repeating_denominators(column_name_list, spec_dict, delimiter)
	...
	if 'all' in test_list or 'multiple_measurement' in test_list:
		temp_list = find_multiple_measurement(column_name_list, spec_dict, delimiter)
...
def run_tests_column_dict(columns_dict, spec_dict, test_list=['all'], output_path='../output/', filewise=False, show_summary=False, delimiter='!!'):
	
	test_results = {}
	for filename in columns_dict:
		if filename != 'all':
			cur_columns = columns_dict[filename]['column_list']
			columns_dict[filename]['ignored_column_list'] = ignored_columns(cur_columns, spec_dict, delimiter)
			columns_dict[filename]['accepted_column_list'] = remove_columns_to_be_ignored(cur_columns, spec_dict, delimiter)
			columns_dict[filename]['accepted_token_list'] = get_tokens_list_from_column_list(columns_dict[filename]['accepted_column_list'], delimiter)
			columns_dict[filename]['column_list_count'] = len(cur_columns)
			columns_dict[filename]['ignored_column_count'] = len(columns_dict[filename]['ignored_column_list'])
			columns_dict[filename]['accepted_column_count'] = len(columns_dict[filename]['accepted_column_list'])
			...
			if filewise:
				...
				print(filename)
				print('----------------------------------------------------')
				test_results[filename] = test_column_name_list(cur_columns, spec_dict, test_list, True, delimiter)
				print('Total Number of Columns', columns_dict[filename]['column_list_count'])
				print('Total Number of Ignored Columns', columns_dict[filename]['ignored_column_count'])
				print('Total Number of Accepted Columns', columns_dict[filename]['accepted_column_count'])
	...
	if not filewise or show_summary:
		test_results['all'] = test_column_name_list(all_columns, spec_dict, test_list, False, delimiter)
	test_results['all'].update(test_spec(all_columns, spec_dict, test_list, delimiter))

	
	columns_dict['all']['ignored_column_list'] = ignored_columns(all_columns, spec_dict, delimiter)
	columns_dict['all']['accepted_column_list'] = remove_columns_to_be_ignored(all_columns, spec_dict, delimiter)
	columns_dict['all']['accepted_token_list'] = get_tokens_list_from_column_list(columns_dict['all']['accepted_column_list'], delimiter)
	columns_dict['all']['column_list_count'] = len(all_columns)
	columns_dict['all']['ignored_column_count'] = len(columns_dict['all']['ignored_column_list'])
	columns_dict['all']['accepted_column_count'] = len(columns_dict['all']['accepted_column_list'])

	print('Total Number of Columns', columns_dict['all']['column_list_count'])
	print('Total Number of Ignored Columns', columns_dict['all']['ignored_column_count'])
	print('Total Number of Accepted Columns', columns_dict['all']['accepted_column_count'])
...
def test_CSVfile_list(csv_path_list, spec_path, test_list=['all'], output_path='../output/', filewise=False, show_summary=False, is_metadata = [False], delimiter='!!'):
	...
	run_tests_column_dict(columns_dict, spec_dict, test_list, output_path, filewise, show_summary, delimiter)
	

#TODO this will overwrite outputs if filenames repeat across zip files
def test_zip_file_list(zip_path_list, spec_path, test_list=['all'], output_path='../output/', filewise=False, show_summary=False, check_metadata = False, delimiter='!!'):
	# clean the file paths
	spec_path = os.path.expanduser(spec_path)
	output_path = os.path.expanduser(output_path)
	if not os.path.exists(output_path):
...
def test_column_list(column_list_path, spec_path, test_list=['all'], output_path='../output/', delimiter='!!'):
	# clean the file paths
	...
	run_tests_column_dict(columns_dict, spec_dict, test_list, output_path, False, True, delimiter)
