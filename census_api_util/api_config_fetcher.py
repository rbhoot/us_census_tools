    ...
    for dataset_dict in datasets['dataset']:
      ...
      if dataset_dict['c_isAvailable']:
        ...
        if 'c_vintage' in dataset_dict:
          ...
          link_tree_dict[link_tree]['years'][year]['title'] = dataset_dict[
              'title']
          ...
          if 'http_err_code' not in group_list:
            ...
            for cur_group in group_list:
              ...
              if cur_group['variables'] != generate_url_group_variables(
                ...
                error_dict['url_mismatch'].append({
                    'expected':
                        generate_url_group_variables(link_tree,
                                                     cur_group['name'], year),
                    'actual':
                        cur_group['variables']
                })
          ...
          if 'http_err_code' not in geo_list:
            ...
            if 'fips' in geo_list:
              ...
              for cur_geo in geo_list:
                if cur_geo['name'] not in link_tree_dict[link_tree]['years'][
                    year]['geos']:
                  link_tree_dict[link_tree]['years'][year]['geos'][
                      cur_geo['name']] = {}
                  if 'geoLevelDisplay' in cur_geo:
                    link_tree_dict[link_tree]['years'][year]['geos'][cur_geo[
                        'name']]['geoLevelId'] = cur_geo['geoLevelDisplay']
                  elif 'geoLevelId' in cur_geo:
                    link_tree_dict[link_tree]['years'][year]['geos'][
                        cur_geo['name']]['geoLevelId'] = cur_geo['geoLevelId']
                  else:
                  ...
                  if 'requires' in cur_geo:
                    link_tree_dict[link_tree]['years'][year]['geos'][
                        cur_geo['name']]['requires'] = cur_geo['requires']
                  if 'wildcard' in cur_geo:
                    link_tree_dict[link_tree]['years'][year]['geos'][
                        cur_geo['name']]['wildcard'] = cur_geo['wildcard']
                  if 'limit' in cur_geo:
                    link_tree_dict[link_tree]['years'][year]['geos'][
                        cur_geo['name']]['limit'] = cur_geo['limit']
                  # check only 3 key values
                  if len(cur_geo) > 7:
                    if 'groups_extra_keys' not in error_dict:
                      error_dict['groups_extra_keys'] = []
          ...
          if 'fips' in geo_list:
            ...
            for cur_geo in geo_list:
              if cur_geo['name'] not in link_tree_dict[link_tree]['years'][
                ...
                if 'geoLevelDisplay' in cur_geo:
                  link_tree_dict[link_tree]['geos'][cur_geo['name']][
                      'geoLevelId'] = cur_geo['geoLevelDisplay']
                elif 'geoLevelId' in cur_geo:
                  link_tree_dict[link_tree]['geos'][
                      cur_geo['name']]['geoLevelId'] = cur_geo['geoLevelId']
                else:
