    ...
    if os.path.isfile(os.path.join(store_path, 'dataset_year.json')):
        ...
        for dataset_dict in datasets['dataset']:
            ...
            if dataset_dict['c_isAvailable']:
                ...
                elif 'c_isTimeseries' in dataset_dict and dataset_dict[
                        'c_isTimeseries']:
                    year = None

                    if 'title' not in link_tree_dict[link_tree]:
                        link_tree_dict[link_tree]['title'] = dataset_dict[
                            'title']
                    elif link_tree_dict[link_tree]['title'] != dataset_dict[
                            'title']:
                ...
                if dataset_dict['c_groupsLink'] != generate_url_groups(
                        link_tree, year):
                    if 'url_mismatch' not in error_dict:
                        error_dict['url_mismatch'] = []
                    error_dict['url_mismatch'].append({
                ...
                if 'c_tagsLink' in dataset_dict:
                    if dataset_dict['c_tagsLink'] != generate_url_tags(
                            link_tree, year):
                        if 'url_mismatch' not in error_dict:
                            error_dict['url_mismatch'] = []
                        error_dict['url_mismatch'].append({
...
def fetch_linktree_config_cache(param,
    ...
    if param == 'group_variables':
        link_tree_dict = compile_non_group_variables_map(
            store_path, force_fetch)
    error_dict = {}
    url_list = {}
    ...
    for link_tree in link_tree_dict:
        if 'years' in link_tree_dict[link_tree]:
            for year in link_tree_dict[link_tree]['years']:
                ...
                if param == 'group_variables':
                    for group_id in link_tree_dict[link_tree]['years'][year][
                            'groups']:
                        temp_url = generate_url_group_variables(
                            link_tree, group_id, year)
    ...
    error_dict = dowload_url_list_parallel(status_file,
                                           chunk_size=50,
                                           chunk_delay_s=0.8)
...
def compile_groups_map(store_path=config_path_, force_fetch=False):
    if os.path.isfile(os.path.join(store_path,
        ...
        for link_tree in link_tree_dict:
            if 'years' in link_tree_dict[link_tree]:
                for year in link_tree_dict[link_tree]['years']:
                    cache_file = os.path.join(cache_path, link_tree, str(year),
                                              'groups.json')
                    temp_url = generate_url_groups(link_tree, year)
                    ...
                    if 'http_err_code' not in group_list:
                        ...
                        for cur_group in group_list:
                            ...
                            if cur_group[
                                ...
                                print(link_tree,
                                      'group_variablesLink unexpected')
    ...
    if os.path.isfile(os.path.join(
        ...
        for link_tree in link_tree_dict:
            if 'years' in link_tree_dict[link_tree]:
                    ...
                    if 'http_err_code' not in geo_list:
                        ...
                        if 'fips' in geo_list:
                            ...
                            for cur_geo in geo_list:
                                if cur_geo['name'] not in link_tree_dict[
                                        link_tree]['years'][year]['geos']:
                                    link_tree_dict[link_tree]['years'][year][
                                        'geos'][cur_geo['name']] = {}
                                    if 'geoLevelDisplay' in cur_geo:
                                        link_tree_dict[link_tree]['years'][
                                            year]['geos'][cur_geo['name']][
                                                'geoLevelId'] = cur_geo[
                                                    'geoLevelDisplay']
                                    ...
                                    error_dict['geo_conflicts'].append('_'.join(
                                        [link_tree, year, cur_geo['name']]))
                ...
                cache_file = os.path.join(cache_path, link_tree,
                                          'geography.json')
                temp_url = generate_url_geography(link_tree)
                if os.path.isfile(cache_file):
                    geo_list = json.load(open(cache_file, 'r'))
                ...
                if 'http_err_code' not in geo_list:
                    ...
                    if 'fips' in geo_list:
                        ...
                        for cur_geo in geo_list:
                            if cur_geo['name'] not in link_tree_dict[link_tree][
                                    'years'][year]['geos']:
                                link_tree_dict[link_tree]['geos'][
                                    cur_geo['name']] = {}
                                if 'geoLevelDisplay' in cur_geo:
                                    link_tree_dict[link_tree]['geos'][cur_geo[
                                        'name']]['geoLevelId'] = cur_geo[
                                            'geoLevelDisplay']
                                elif 'geoLevelId' in cur_geo:
                                    link_tree_dict[link_tree]['geos'][cur_geo[
                                        'name']]['geoLevelId'] = cur_geo[
                                ...
                                if 'requires' in cur_geo:
                                    link_tree_dict[link_tree]['geos'][
                                        cur_geo['name']]['requires'] = cur_geo[
                                            'requires']
                                if 'wildcard' in cur_geo:
                                    link_tree_dict[link_tree]['geos'][
                                        cur_geo['name']]['wildcard'] = cur_geo[
                                            'wildcard']
                                if 'limit' in cur_geo:
                                    link_tree_dict[link_tree]['geos'][cur_geo[
                                        'name']]['limit'] = cur_geo['limit']
                                # check only 3 key values
                                if len(cur_geo) > 7:
                                    if 'groups_extra_keys' not in error_dict:
                                        error_dict['groups_extra_keys'] = []
    ...
    if os.path.isfile(
        ...
        for link_tree in link_tree_dict:
            if 'years' in link_tree_dict[link_tree]:
                ...
                cache_file = os.path.join(cache_path, link_tree, year,
                                          'variables.json')
                temp_url = generate_url_variables(link_tree)
                if os.path.isfile(cache_file):
                    variable_list = json.load(open(cache_file, 'r'))
...
def compile_linktree_group_map(store_path=config_path_, force_fetch=False):
    if os.path.isfile(os.path.join(store_path,
                                   'linktree_groups.json')) and not force_fetch:
        out_dict = json.load(
            open(os.path.join(store_path, 'linktree_groups.json'), 'r'))
    else:
        link_tree_dict = compile_non_group_variables_map(
            store_path, force_fetch)
        out_dict = {}
...
def compile_linktree_group_years_map(store_path=config_path_,
                                     force_fetch=False):
    if os.path.isfile(os.path.join(
            store_path, 'linktree_years_groups.json')) and not force_fetch:
        out_dict = json.load(
            open(os.path.join(store_path, 'linktree_years_groups.json'), 'r'))
    else:
        link_tree_dict = compile_non_group_variables_map(
            store_path, force_fetch)
        out_dict = {}
        ...
        with open(os.path.join(store_path, 'linktree_years_groups.json'),
                  'w') as fp:
            json.dump(out_dict, fp, indent=2)
