...
# get column list
def get_column_list_all(link_tree,
                        group_id,
                        config_path=config_path_,
                        store_path='~/acs_tables/'):
  ...
  if len(link_tree_dict[link_tree]['groups'][group_id]) > 0:
    for year in link_tree_dict[link_tree]['groups'][group_id]:
      cache_file = os.path.join(config_path, 'api_cache', link_tree, str(year),
                                group_id + '.json')
      ...
      if 'http_err_code' not in group_variables_dict:
        if 'variables' not in group_variables_dict:
          ...
          ret_dict[year] = []
          for variable_id, variable_dict in group_variables_dict[
              'variables'].items():
    ...
    cache_file = os.path.join(config_path, 'api_cache', link_tree,
                              group_id + '.json')
    if os.path.isfile(cache_file):
      group_variables_dict = json.load(open(cache_file, 'r'))
    else:
...
def get_column_list_years(link_tree,
                          group_id,
                          year_list,
                          config_path=config_path_,
                          store_path='~/acs_tables/'):
  group_id = group_id.upper()
  columns_dict = get_column_list_all(link_tree, group_id, config_path,
                                     store_path)
  ret_dict = {}
  ret_dict['all'] = []
