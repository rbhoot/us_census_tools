...
def fetch_dcid_properties_enums(class_dcid, output_path=module_dir_+'/prefetched_outputs', force_fetch=False):
	...
	if force_fetch or not os.path.isfile(os.path.join(output_path, f'{class_dcid}_dc_props.json')):
		population_props = request_url_json(f'https://autopush.api.datacommons.org/node/property-values?dcids={class_dcid}&property=domainIncludes&direction=in')
		...
		with open(os.path.join(output_path, f'{class_dcid}_dc_props.json'), 'w') as fp:
			json.dump(dc_props, fp, indent=2)
	else:
	    dc_props = json.load(open(os.path.join(output_path, f'{class_dcid}_dc_props.json'), 'r'))
		


	# check if the list has enum type
	if force_fetch or not os.path.isfile(os.path.join(output_path, f'{class_dcid}_dc_props_types.json')):
		population_props_types = request_url_json(f'https://autopush.api.datacommons.org/node/property-values?dcids={"&dcids=".join(dc_props.keys())}&property=rangeIncludes&direction=out')
		...
		with open(os.path.join(output_path, f'{class_dcid}_dc_props_types.json'), 'w') as fp:
			json.dump(dc_props, fp, indent=2)
					
		# print(population_props_enums)
	else:
	    dc_props = json.load(open(os.path.join(output_path, f'{class_dcid}_dc_props_types.json'), 'r'))

	# get enum value list
	if force_fetch or not os.path.isfile(os.path.join(output_path, f'{class_dcid}_dc_props_enum_values.json')):
		...
		for property_name in new_dict.keys():
			...
			for type_name in new_dict[property_name]:
				if 'enum' in type_name.lower():
					enum_values = request_url_json(f'https://autopush.api.datacommons.org/node/property-values?dcids={type_name}&property=typeOf&direction=in')
		...
		with open(os.path.join(output_path, f'{class_dcid}_dc_props_enum_values.json'), 'w') as fp:
			json.dump(dc_props, fp, indent=2)
	else:
	    dc_props = json.load(open(os.path.join(output_path, f'{class_dcid}_dc_props_enum_values.json'), 'r'))
