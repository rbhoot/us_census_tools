                                        ...
                                        if 'Margin of Error' not in column_name_list[
                                                i] and 'Median' not in column_name_list[
                                                    i] and 'Mean' not in column_name_list[
                                                        i] and 'INCOME' not in column_name_list[
                                                            i]:
                                            if column_name_list[
                                                    i] not in total_columns:
...
for column_name in total_columns.copy():
    ...
    for new_token in replace_columns_names:
        other_column = replace_token_in_column(
            column_name, 'Total group quarters population', new_token,
            delimiter)
    ...
    if 'Median' not in cur_column and 'Mean' not in cur_column:
        ...
        if prefix:
            ...
            for column_group in column_names:
                temp_str = replace_token_in_column(cur_column, 'Estimate',
                                                   'Margin of Error', delimiter)
                temp_str = replace_first_token_in_column(
                    temp_str, column_group, column_group + ' MOE', delimiter)
                if temp_str in all_columns and temp_str not in denominators[
                        prefix]:
...
print(
    json.dumps(list(set(median_moe_columns).union(set(mean_moe_columns))),
               indent=2))
