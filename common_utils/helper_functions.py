...
def replace_token_in_column(cur_column, old_token, new_token, delimiter='!!'):
	return delimiter.join([new_token if x==old_token else x for x in cur_column.split(delimiter)])

def replace_first_token_in_column(cur_column, old_token, new_token, delimiter='!!'):
	new_list = []
	temp_flag = True
...
# replace token
def replace_token_in_column_list(column_list, old_token, new_token, delimiter='!!'):
	ret_list = []
	for cur_column in column_list:
		ret_list.append(replace_token_in_column(cur_column, old_token, new_token, delimiter))
	return ret_list

# combined replace list
def replace_token_list_in_column_list(column_list, old_token, new_token_list, delimiter='!!'):
	ret_dict = {}
	for cur_column in column_list:
		ret_dict[cur_column] = []
		for new_token in new_token_list:
			ret_dict[cur_column].append(replace_token_in_column(cur_column, old_token, new_token, delimiter))
	return ret_dict

# find columns sub token
def find_columns_with_token_partial_match(column_list, token_str, delimiter='!!'):
	ret_list = []
	for cur_column in column_list:
...
def get_columns_with_same_prefix(columns_by_length, max_extra_token=1, delimiter='!!'):
	ret_dict = {}
					...
					for row in csv_reader:
						if csv_reader.line_num == 2:
							...
							for i, val in enumerate(row):
								try:
									if float(val) > 100:
										if 'Margin of Error' not in column_name_list[i] and 'Median' not in column_name_list[i] and 'Mean' not in column_name_list[i] and 'INCOME' not in column_name_list[i]:
...
total_columns = guess_total_columns_from_zip_file('~/acs_tables/S2602/S2602_us.zip')
...
total_columns = find_columns_with_token(total_columns, 'Total group quarters population')
...
for column_name in total_columns.copy():
	...
	for new_token in replace_columns_names:
		other_column = replace_token_in_column(column_name, 'Total group quarters population', new_token, delimiter)
...
for cur_column in all_columns:
	...
	if 'Median' not in cur_column and 'Mean' not in cur_column:
		...
		if prefix:
			...
			temp_str = replace_token_in_column(cur_column, 'Estimate', 'Margin of Error', delimiter)
			if temp_str not in all_columns:
				print('Column expected but not found:', temp_str)
			for column_group in column_names:
				temp_str = replace_token_in_column(cur_column, 'Estimate', 'Margin of Error', delimiter)
				temp_str = replace_first_token_in_column(temp_str, column_group, column_group+' MOE', delimiter)
...
median_moe_columns = find_columns_with_token_partial_match(total_moe_columns, 'Median')
mean_moe_columns = find_columns_with_token_partial_match(total_moe_columns, 'Mean')
...
median_moe_columns =find_columns_with_token_partial_match(median_columns, 'Margin of Error')
...
for column_name in total_columns:
	...
	for token in total_replacements:
		...
		new_column_moe = replace_token_in_column(column_name, 'Total', token+' MOE')
		if new_column in all_columns:
			denominators_section[column_name].append(new_column)
		temp_str = replace_token_in_column(new_column, 'Estimate', 'Margin of Error')
		if temp_str in all_columns:
			denominators_section[column_name].append(temp_str)
		temp_str = replace_token_in_column(new_column_moe, 'Estimate', 'Margin of Error')
		if temp_str in all_columns:
			denominators_section[column_name].append(temp_str)
...
denominators_section.update(replace_token_list_in_column_list(total_moe_columns, 'Total MOE', ['Moved; within same municipio MOE', 'Moved; from different municipio MOE', 'Moved; from the U.S. MOE', 'Moved; from outside Puerto Rico and the U.S. MOE']))
...
replaced_columns = replace_token_in_column_list(median_est, 'Estimate', 'Margin of Error')
...
total_columns = list(set(total_columns)-set(median_columns))
replaced_columns = replace_token_in_column_list(total_columns, 'Estimate', 'Margin of Error')
moe_columns = find_columns_with_token(all_columns, 'Margin of Error')
