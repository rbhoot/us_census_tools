...
def get_tokens_list_from_zip(zip_file_path, check_metadata = False, print_details=False, delimiter='!!'):
	# tokens = set()
...
def column_to_be_ignored(column_name, spec_dict, delimiter='!!'):
	...
	if 'ignoreColumns' in spec_dict:
		for ignore_token in spec_dict['ignoreColumns']:
			if delimiter in ignore_token and set(ignore_token.split(delimiter)).issubset(column_name.split(delimiter)):
				ret_value = True
			elif token_in_list_ignore_case(ignore_token, column_name.split(delimiter)):
				ret_value = True
	return ret_value
...
def get_spec_token_list(spec_dict, delimiter='!!'):
	...
	return {'token_list':list(set(ret_list)), 'repeated_list':list(set(repeated_list))}
